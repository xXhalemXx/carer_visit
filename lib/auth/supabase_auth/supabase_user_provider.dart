...
class CarerVisitsSupabaseUser extends BaseAuthUser {
  CarerVisitsSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> carerVisitsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CarerVisitsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
